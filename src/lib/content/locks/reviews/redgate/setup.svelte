...
<ol class="mb-8 ml-4 list-disc space-y-3">
	...
	<li>
		...
		servers. Instead, it SSH's in to collect machine level metrics. It also connects to the Postgres
		server directly
	</li>
	<li>
		The monitoring software must store data somewhere. You have to host a Postgres instance running
		timescale or SQL Server to store the software's system.
	</li>
</ol>

<p class="p">
	It may be necessary to self-host the website, monitoring database, and metrics tracker on separate
	machines, depending on load. The configuration recommendations are outlined on their
	<a
...
scp &lt;path_to_downloaded_file&gt; root@&lt;YOUR_VM's_IP&gt;:/root/
		...
		<p class="p">
			Alternatively, you can try to mail it to your VM or use another program to transfer the files,
			but SCP is the fastest approach.
		</p>
		<h4 class="mt-8 mb-4 text-lg font-bold text-stone-900">Unzip the Monitor file</h4>
		<p class="p">If you didn't unzip the files on your local machine, install the unzip utility:</p>
		<!-- prettier-ignore  -->
		<CodeBlock>
sudo apt update
...
<DropDown title="Part 5: Configuring the site">
	<section class="explainer_section">
		...
		<h4 class="mt-8 mb-4 text-lg font-bold text-stone-900">Select Postgres as your server type</h4>
